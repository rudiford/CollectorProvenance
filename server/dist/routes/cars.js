...
router.get("/", async (req, res) => {
    try {
        ...
        const rawResults = await query.orderBy(desc(cars.createdAt)).limit(parseInt(limit)).offset(parseInt(offset));
        ...
        if (q) {
            ...
            filtered = results.filter((c) => c.model?.toLowerCase().includes(search) ||
        ...
        if (model)
            filtered = filtered.filter((c) => c.model?.toLowerCase() === model.toLowerCase());
        if (year)
            filtered = filtered.filter((c) => c.year === parseInt(year));
        if (status)
            filtered = filtered.filter((c) => c.currentStatus === status);
        if (country)
            filtered = filtered.filter((c) => c.locationCountry === country);
...
router.post("/", requireAuth, async (req, res) => {
    try {
        const { chassisNumber, year, make, model, variant, engineNumber, transmissionNumber, factoryColorCode, factoryColorName, factoryInterior, factoryOptions, currentStatus, locationState, locationCountry, isPublic, willingToSell, } = req.body;
        if (!year || !model) {
