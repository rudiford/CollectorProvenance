...
router.post("/car/:carId", async (req, res) => {
    try {
        ...
        if (!car.isPublic)
            return res.status(403).json({ error: "This car is not publicly listed" });
...
router.patch("/:id/read", requireAuth, async (req, res) => {
    try {
        ...
        if (msg.ownerId !== req.session.userId)
...
async function sendNotificationEmail(toEmail, data) {
