...
import { transferCodes, cars, ownershipRecords, users } from "../db/schema.js";
...
router.post("/generate/:carId", requireAuth, async (req: Request, res: Response) => {
  try {
...
router.post("/claim", requireAuth, async (req: Request, res: Response) => {
  try {
    const { code } = req.body;
    if (!code) return res.status(400).json({ error: "Transfer code is required" });
    ...
    await db
      ...
      .set({ toDate: today })
      .where(and(eq(ownershipRecords.carId, transfer.carId), eq(ownershipRecords.toDate, null as any)));
...
router.post("/cancel/:transferId", requireAuth, async (req: Request, res: Response) => {
  try {
    const [transfer] = await db
      .select()
      .from(transferCodes)
...
router.get("/car/:carId", requireAuth, async (req: Request, res: Response) => {
  try {
    const [car] = await db.select().from(cars).where(eq(cars.id, req.params.carId)).limit(1);
    if (!car) return res.status(404).json({ error: "Car not found" });
    if (car.currentOwnerId !== req.session.userId) {
