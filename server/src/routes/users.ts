...
async function buildProfileResponse(req: Request, res: Response, user: any) {
  ...
  if (isOwnProfile) {
    ...
    if (user.showEmail) publicUser.email = user.email;
    if (user.showPhone) publicUser.phone = user.phone;
    if (user.showCity) publicUser.locationCity = user.locationCity;
    if (user.showState) publicUser.locationState = user.locationState;
    if (user.showCountry) publicUser.locationCountry = user.locationCountry;
    if (user.showWebsite) publicUser.website = user.website;
    if (user.showInstagram) publicUser.instagram = user.instagram;
...
router.patch("/me/profile", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    const {
      ...
      showEmail, showPhone, showCity, showState, showCountry, showWebsite, showInstagram, showIdentity,
    } = req.body;
